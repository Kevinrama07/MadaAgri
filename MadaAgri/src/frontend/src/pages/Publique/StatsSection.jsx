...
import styles from '../../styles/Publique/StatsSection.module.css';
...
export default function StatsSection() {
  ...
  return (
    <section className={clsx(styles['stats-section'])} ref={sectionRef}>
      <div className={clsx(styles['stats-container'])}>
        <h2 className={clsx(styles['stats-title'])}>Chiffres clés</h2>
        ...
        <div className={clsx(styles['stats-grid'])}>
          {stats.map((stat) => (
            <div key={stat.id} className={clsx(styles['stat-card'])}>
              ...
              <p className={clsx(styles['stat-label'])}>{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
