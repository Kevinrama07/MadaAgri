...
export const dataApi = {
  ...
  async fetchRegionCultures(regionId) {
    const data = await apiFetch(`/region_cultures?regionId=${encodeURIComponent(regionId)}`);
    return data.region_cultures;
  },

  async fetchDeliveries(farmerId) {
    const data = await apiFetch(`/deliveries?farmerId=${encodeURIComponent(farmerId)}`);
  ...
  async followUser(userId) {
    const data = await apiFetch(`/follows/${encodeURIComponent(userId)}`, { method: 'POST' });
    return data;
  },

  async unfollowUser(userId) {
    const data = await apiFetch(`/follows/${encodeURIComponent(userId)}`, { method: 'DELETE' });
    return data;
  },
  ...
  async fetchKnnCultures(regionId, k = 5) {
    const data = await apiFetch(`/analysis/cultures_knn?regionId=${encodeURIComponent(regionId)}&k=${encodeURIComponent(k)}`);
